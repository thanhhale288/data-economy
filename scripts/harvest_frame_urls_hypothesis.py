...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--per-division", type=int, default=40)
...
def main() -> int:
    args = parse_args()
    sample = sample_frame_for_url_finder(per_division=args.per_division)
    kept: list[dict] = []
    for row in sample:
        identity = {
        ...
        kept.append(
            {
                ...
                "website_url": url,
                "notes": "domain_hypothesis_dns" if not args.no_resolve else "domain_hypothesis",
            }
        )
    ...
    print(json.dumps({"written": len(kept), "path": str(args.out)}, ensure_ascii=False))
    return 0
